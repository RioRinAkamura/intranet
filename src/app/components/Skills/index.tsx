...
import { ConsoleSqlOutlined, PlusOutlined, UnorderedListOutlined } from '@ant-design/icons';
...
export const Skills = memo((props: Props) => {
  ...
  const handleOk = async (items) => {
    ...
    if(data.length === 0) {
      ...
      try {
        const deleteArr = await deleteItems.map((i) => fakeAPI.delete(`/hr/employees/${id}/skills/${i}`))
  ...
  const handleDragEnd = result => {
    ...
    const newList = reorder(skills, result.source.index, result.destination.index);
  ...
  return (
        <>
          <FlexWrapper>
            <span>
              { t(UserDetailMessages.formSkillLabel()) }
            </span>
            { isEdit && <IconBtn onClick={handleToggleModal} icon={<PlusOutlined />} /> }
          </FlexWrapper>
          <SkillsModal
          ...
          <DragDropContext onDragEnd={handleDragEnd}>
            <Droppable droppableId="droppable">
              {
                (provided, snapshot) => {
                  const style = snapshot.isDraggingOver ? { "pointer-events": "none" } :{};
                  return (
                                ...
                                (provided, snapshot) => (
                                  ...
                                  >
                                    <FlexWrapper className="justify-content-between">
                                      <FlexWrapper>
                                        <span className="skill-title">{skill.skill.name}</span>

                                        <Rate
                                          onChange={(value) => handleSkillRateChange(value, skill)}
                                          defaultValue={skill.level} />
                                      </FlexWrapper>
                                      <FlexWrapper>
                                        <DeleteOutlined onClick={() => handleDeleteSkill(skill)} />
